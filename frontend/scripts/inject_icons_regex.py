...
pattern_1 = re.compile(
    r'<div className="flex items-center gap-1 shrink-0 bg-emerald-950/90 px-1 py-0\.5 rounded opacity-0 group-hover:opacity-100 transition-opacity">.*?<Trash2 className="w-3 h-3" />\s*</button>\s*</div>',
    re.DOTALL
)

new_1 = """<div className="flex items-center gap-1 shrink-0 bg-emerald-950/90 px-1 py-0.5 rounded transition-opacity opacity-40 group-hover:opacity-100">
...
content, count1 = pattern_1.subn(new_1, content)
print(f"Replaced {count1} instances for File 1")

# FOR FILE 2
pattern_2 = re.compile(
    r'<div className="flex items-center gap-1 shrink-0 bg-sky-950/90 px-1 py-0\.5 rounded opacity-0 group-hover:opacity-100 transition-opacity">.*?<Trash2 className="w-3 h-3" />\s*</button>\s*</div>',
    re.DOTALL
)

new_2 = """<div className="flex items-center gap-1 shrink-0 bg-sky-950/90 px-1 py-0.5 rounded transition-opacity opacity-40 group-hover:opacity-100">
...
content, count2 = pattern_2.subn(new_2, content)
print(f"Replaced {count2} instances for File 2")
